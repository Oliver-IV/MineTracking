...
import { PATTERNS, RMQ_CLIENT_NAME } from 'configs/rmq.config';

@Injectable()
export class TrafficLightsService {
  constructor(
    @InjectRepository(TrafficLightEntity) private trafficLightRepository: Repository<TrafficLightEntity>,
  ...
  async create(createTrafficLightDto: CreateTrafficLightDto): Promise<TrafficLight> {
    ...
    let location: LocationEntity | null = await this.locationService.findByCoordinates(latitude, longitude);
  ...
  async findAll(): Promise<TrafficLights> {
    const trafficLights : TrafficLight[]= await this.trafficLightRepository.find({});
    return {trafficLights};
  }

  async findOne(trafficLightId: string): Promise<TrafficLight> {
    const trafficLight: TrafficLightEntity | null = await this.trafficLightRepository.findOne({ where: { trafficLightId } });
  ...
  async update(trafficLightId: string, updateTrafficLightDto: UpdateTrafficLightDto): Promise<TrafficLight> {
    ...
    const updated = await this.trafficLightRepository.save({...trafficLight,...updateTrafficLightDto}); 
  ...
  async changeState(changeLightStateDto: ChangeLightStateDto): Promise<TrafficLight> {
    const {trafficLightId, state} = changeLightStateDto;
    const trafficLight: TrafficLight = await this.findOne(trafficLightId);

    const updated = await this.trafficLightRepository.save({...trafficLight,state});
    return updated;
  }
